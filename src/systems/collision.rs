    ...
    for (bullet_entity, bullet_transform) in bullet_query.iter() {
        for (enemy_entity, enemy_transform) in enemy_query.iter() {
            ...
            if distance < (BULLET_SIZE + ENEMY_SIZE) / 2.0 {
                ...
                spawn_explosion(&mut commands, enemy_transform.translation, Color::srgb(1.0, 0.6, 0.0));
    ...
    if let Ok((player_entity, player_transform)) = player_query.single() {
        for (bullet_entity, bullet_transform) in bullet_query.iter() {
            ...
            if distance < (BULLET_SIZE + PLAYER_SIZE) / 2.0 {
                ...
                spawn_explosion(&mut commands, player_transform.translation, Color::srgb(1.0, 0.3, 0.0));
