...
## Project Overview
Vibe Invaders is a Space Invaders clone implemented in **Rust 2024 edition** using the **Bevy 0.17** game engine, following Entity Component System (ECS) architecture principles.
...
- **Test in Release Mode**: Always verify gameplay changes with `cargo run --release`
...
1. **Check First**: Run `cargo check` before making changes
2. **Build Incrementally**: Test with `cargo build` or `cargo check` after modifications
3. **Run the Game**: Use `cargo run --release` to test gameplay
...
## Key Technologies
- **Rust Edition**: 2024
- **Bevy Version**: 0.17 (with minimal feature flags)
- **Dependencies**: `bevy = "0.17"`, `rand = "0.8"`
...
- Test gameplay impact with `cargo run --release`
